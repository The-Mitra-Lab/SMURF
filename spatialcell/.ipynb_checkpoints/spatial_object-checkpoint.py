...
def knn(spots_result,cell_ids,cells_main,start_row_spot,start_col_spot,index_toset):
...
class spatial_object:
    ...
    def __init__(self, image,df,df_temp,start_row_spot,end_row_spot,start_col_spot,end_col_spot,
                    row_left,row_right,col_up,col_down,pixels):
    ...
    def image_temp(self):
        ...
        return self.image[self.row_left: self.row_right, self.col_up: self.col_down,:]


    def add_segmentation(self, segmentation_results, i_max, j_max, loop, gap):
        
        self.segmentation_final = add_segmentation_temp(segmentation_results, i_max, j_max, loop, gap)
        

    def generate_cell_spots_information(self, max_pixel = 50, cells_main_pct = float(1/6)):
        ...
        self.cells_main = create_cells_main(self.cells,self.spots,cells_main_pct,max_pixel)
        ...
        for i in range(len(indexes)):
            self.set_toindex[(int(indexes[i].split("_")[2]),int(indexes[i].split("_")[3].split("-")[0]))] = i
            self.index_toset[i] = (int(indexes[i].split("_")[2]),int(indexes[i].split("_")[3].split("-")[0]))

        self.spots_result = np.zeros([self.end_row_spot - self.start_row_spot,self.end_col_spot - self.start_col_spot])
        self.cell_ids = np.sort(np.array(list(self.cells_main.keys())))

        self.cell_centers, self.distances, self.indices = knn(self.spots_result,self.cell_ids,self.cells_main,self.start_row_spot,self.start_col_spot,self.index_toset)
