...
def prepare_dataframe_image(df_path: 'square_002um/spatial/tissue_positions.parquet',
    ...
    avg_row = (df['pxl_row_in_fullres'].max() - df['pxl_row_in_fullres'].min())/(2*row_number)
    avg_col = (df['pxl_col_in_fullres'].max() - df['pxl_col_in_fullres'].min())/(2*col_nuber)
    ...
    df_temp = df[(df['array_row'] >= start_row_spot ) & (df['array_row'] < end_row_spot ) &
                 (df['array_col'] >= start_col_spot ) & (df['array_col'] < end_col_spot )].copy()
    ...
    df_temp.loc[:, 'pxl_row_left_in_fullres_temp'] = df_temp.loc[:, 'pxl_row_left_in_fullres'] -  row_left
    df_temp.loc[:,'pxl_row_right_in_fullres_temp'] = df_temp.loc[:,'pxl_row_right_in_fullres'] -  row_left
    df_temp.loc[:,'pxl_col_up_in_fullres_temp'] = df_temp.loc[:,'pxl_col_up_in_fullres'] -  col_up
    df_temp.loc[:,'pxl_col_down_in_fullres_temp'] = df_temp.loc[:,'pxl_col_down_in_fullres'] -  col_up
    ...
    return spatial_object(image_array,df,df_temp,start_row_spot,end_row_spot,start_col_spot,end_col_spot, row_left,row_right,col_up,col_down,pixels)
...
def nuclei_rna(adata,so,min_percent = 0.4):
    ...
    for i in range(len(indexes)):
        set_toindex_data[(int(indexes[i].split("_")[2]),int(indexes[i].split("_")[3].split("-")[0]))] = i
    ...
    for cell_id in tqdm.tqdm(cell_ids):
        ...
        for spot_id in so.cells_main[cell_id]:
            ...
            set_cell.add((so.df.loc[spot_id,'array_row'],so.df.loc[spot_id,'array_col']))
        ...
        set_cell = {tup for tup in set_cell if tup[0] >= so.start_row_spot and tup[0] < so.end_row_spot}
        set_cell = {tup for tup in set_cell if tup[1] >= so.start_col_spot and tup[1] < so.end_col_spot}
        ...
        set_tobe = {tup for tup in set_tobe if tup[0] >= so.start_row_spot and tup[0] < so.end_row_spot}
        set_tobe = {tup for tup in set_tobe if tup[1] >= so.start_col_spot and tup[1] < so.end_col_spot}
        ...
        cell_matrix[cell_id]= [set_toindex_data[key] for key in set_cell if key in set_toindex_data]
        cell_matrix[cell_id] = data_temp[cell_matrix[cell_id],:].sum(axis =0).reshape(1, -1)
...
def singlecellanalysis(adata,save = False,i= None,path= None,resolution=1,random_state = 0,show = True):
...
def expanding_cells(so, adata_sc, weights, iter_cells, data_temp,  min_percent = 0.4,cortex = False):
    ...
    def fill_points(points):
        ...
        for x, y in point_set1:
            ...
            for xx,yy in neighbors :
                if ((xx,yy) not in point_set) and ((xx+1,yy) in point_set) and ((xx-1,yy) in point_set) and ((xx,yy+1) in point_set) and ((xx,yy-1) in point_set):
    ...
    for i_num in tqdm.tqdm(range(total_num)):
        ...
        while len(tobe_considered ) !=0:
            for row, col in tobe_considered:
                cell_matrix_temp  = data_temp[so.set_toindex_data[(row,col)],:] + cell_matrix
                #cell_matrix_norm = 10000*cell_matrix_temp/cell_matrix_temp.sum()
    
                score_temp = (np.dot(cell_matrix_temp, weights[cell_type ]) / norm(cell_matrix_temp))[0,0]
                        
                if score_temp > score :# data_temp[set_toindex[(row,col)],:].sum() == 0:
                    #print(score_temp)
                    set_cell.update([(row,col)])
                    cell_matrix = cell_matrix_temp
                    
                    if row-1 >= so.start_row_spot and (row-1,col) in so.set_toindex_data: 
                    ...
                    if row+1 < so.end_row_spot and (row+1,col) in so.set_toindex_data: 
                    ...
                    if col-1 >= so.start_col_spot and (row,col-1) in so.set_toindex_data: 
                    ...
                    if col+1 < so.end_col_spot  and (row,col+1) in so.set_toindex_data: 
        ...
        for i_num in tqdm.tqdm(range(total_num)):
            ...
            if len(points) > 2:
                ...
                try:
                    ...
                    set_cell_index = [so.set_toindex_data[key] for key in cells_final[cell_id] if key in so.set_toindex_data]
                    cell_matrix =  copy.deepcopy(data_temp[set_cell_index,:].sum(axis =0).reshape(1, -1))
                    cellscore[i_num] = np.dot(cell_matrix, weights[cell_type]) / norm(cell_matrix)
...
def plot_celltype_position(cell_type_final, col_num=5):
    ...
    fig, axs = plt.subplots(nrows=rows, ncols=col_num, figsize=(12 * col_num, 12 * rows))
...
def plot_colors_with_indices(ax, colors = None):
    ...
    if colors == None:
        ...
        colors = [(172, 106, 88), (203, 205, 199), (135, 36, 197), (138, 76, 124), (208, 67, 111),    (195, 190, 120), (53, 137, 24), (218, 161, 52), (212, 229, 95), (214, 154, 126),    (201, 147, 47), (96, 35, 35), (38, 116, 230), (69, 170, 54), (223, 96, 36),    (197, 213, 195), (29, 101, 119), (186, 109, 121), (127, 202, 188), (76, 216, 34),    (108, 190, 202), (75, 64, 110), (109, 76, 90), (141, 92, 60), (74, 84, 82),    (45, 166, 110), (93, 149, 201), (100, 68, 27), (167, 158, 108), (157, 151, 133),    (24, 52, 56), (47, 51, 169), (123, 113, 91), (103, 162, 189), (176, 28, 225),    (129, 49, 52), (114, 103, 104), (69, 118, 27), (25, 104, 216), (71, 117, 103),    (200, 110, 26), (143, 48, 142), (163, 211, 182), (140, 149, 222), (223, 68, 30),    (208, 155, 75), (132, 190, 119), (223, 67, 163), (170, 181, 219), (145, 220, 27)]

        

    for i, color in enumerate(colors):
        color_normalized = [x/255 for x in color]
        rect = patches.Rectangle((i, 0), 1, 1, linewidth=1, edgecolor='none', facecolor=color_normalized)
        ax.add_patch(rect)
        ax.text(i + 0.5, 0.5, str(i), color='white' if sum(color_normalized[:3]) < 1.5 else 'black', 
...
def plot_image_with_overlay(ax, so, cell_type_final, colors):
    ...
    if img_np.shape[2] == 3:
        img_np = np.dstack([img_np, np.ones((img_np.shape[0], img_np.shape[1]), dtype=np.uint8) * 255])

    overlay = np.full((img_np.shape[0], img_np.shape[1], 4), [0, 0, 0, 0], dtype=np.uint8)
    ...
    for c in range(4):
        result[:, :, c] = (img_np[:, :, c] * (1 - overlay[:, :, 3] / 255) + overlay[:, :, c] * (overlay[:, :, 3] / 255)).astype(np.uint8)
...
def plot_whole(so, cell_type_final, colors = None, save = 'result_whole_pic.pdf'):
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(20.5, 20), gridspec_kw={'height_ratios': [1, 20]})
...
def itering_arragement(adata_sc, adata_raw, adata, so,resolution=1, save_folder = 'results_example/', show = True):
    ...
    for i in range(1,30):
        ...
        cells_final, final_data, cellscore, length_final = expanding_cells(so, adata_temp, weights , iter_cells, data_temp = csr_matrix(adata.X), min_percent = 0.4,cortex = False)
        adata_temp = anndata.AnnData(X=csr_matrix(final_data),
                            obs=pd.DataFrame([],index = list(adata_sc.obs.index)),
                            var=adata.var)
        adata_temp.write(save_folder +'adatas_ini_'+str(i)+'.h5ad')
        adata_temp = singlecellanalysis(adata_temp,True,i,save_folder,resolution=resolution, show=show)
        adata_type_record[i] = list(adata_temp.obs['leiden'])
        mi = normalized_mutual_info_score(adata_type_record[i] , adata_type_record[i-1])
